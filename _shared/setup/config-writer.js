...
function setPropertyType(type) {
  ...
  // 当设置为 tcm-clinic 类型时，确保必要数据文件存在
  if (type === 'tcm-clinic') {
    initTcmDataFiles();
  }
...
/**
 * 初始化中医馆所需的数据文件（仅在文件不存在时创建）
 */
function initTcmDataFiles() {
  const dataDir = path.join(__dirname, '..', 'data');
  const filesToInit = {
    'members.json': { members: [], membershipTiers: [] },
    'transactions.json': { transactions: [] },
    'inventory.json': { products: [], transactions: [] }
  };

  for (const [filename, defaultContent] of Object.entries(filesToInit)) {
...
/**
 * 更新中医馆基础信息
 * @param {Object} fields - 基础信息字段
 * @param {string} fields.clinicName       诊所名称（必填）
 * @param {string} [fields.address]        地址
 * @param {string} [fields.operatingHours] 营业时间（如 "周一至周六 8:00-18:00"）
 * @param {string} [fields.area]           面积（可选）
 * @param {string} [fields.description]    诊所简介（可选）
 * @param {string[]} [fields.specialties]  特色科室（数组，可选）
 * @returns {{success: boolean, tcm?: object, error?: string}}
 */
function updateTcm(fields) {
  if (!fields || typeof fields !== 'object') return fail('fields 参数必须为对象');

  const config = loadJSON(CONFIG_PATH);
  config.tcm = config.tcm || {};
  Object.assign(config.tcm, fields);

  // 校验必填项：clinicName 必填且非空
  if (!config.tcm.clinicName || String(config.tcm.clinicName).trim() === '') {
    return fail('clinicName（诊所名称）为必填项且不能为空');
  }

  saveJSON(CONFIG_PATH, config);
  touchSetupState();
  return { success: true, tcm: config.tcm };
}
 ...
 * @param {string} [staffData.specialty]   擅长领域（如 "针灸推拿、颈椎病"）
 * @param {string} [staffData.schedule]    坐诊时间（如 "周一、三、五全天"）
 * @param {string} [staffData.qualification] 资质（如 "主治医师"）
 */
...
/**
 * 更新指定步骤的状态（底层方法）
 * @param {number|string} step   步骤号（1-5）
 * @param {string} status        状态（'done' / 'skip' / 'error'）
 * @param {object} [data]        可选元数据
 */
...
/**
 * 显式推进向导步骤（供 Agent 在每步完成后调用）
 *
 * 用法：
 *   cw.advanceStep(1);  // 第 1 步完成
 *   cw.advanceStep(2);  // 第 2 步完成
 *
 * @param {number|string} step  已完成的步骤号（1-5）
 * @returns {{success:boolean, currentStep:number, completed:boolean}}
 */
function advanceStep(step) {
  if (!step || Number(step) < 1 || Number(step) > 5) {
    return fail(`无效步骤号：${step}，仅支持 1-5`);
  }
  const state = updateSetupStep(String(step), 'done');
  console.log(`✅ 向导步骤 ${step} 已完成（当前进度：${state.currentStep}/5）`);
  return { success: true, currentStep: state.currentStep, completed: state.completed || false };
}
