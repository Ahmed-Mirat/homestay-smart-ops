...
function renderTcmBasicInfo(tcm) {
  if (!tcm || !tcm.clinicName) {
    return '';
  }
  let md = `## 基本信息\n\n`;
  if (tcm.address) md += `- 📍 地址：${tcm.address}\n`;
  if (tcm.operatingHours) md += `- ⏰ 营业时间：${tcm.operatingHours}\n`;
  if (tcm.description) md += `- 📝 简介：${tcm.description}\n`;
  return md;
}

function renderTcmKB(data) {
  const tcmInfo = data.tcm || {};
  const name = tcmInfo.clinicName || data.basic?.name || '中医馆';

  // 从 staff.json 读取医师数据，合并到 data 中
  let staffDoctors = [];
  try {
    const staffData = loadJSONSafe(STAFF_PATH);
    staffDoctors = (staffData?.staff || []).filter(s => s.role === 'doctor');
  } catch (err) {
    console.error(`⚠️ 读取 staff.json 失败: ${err.message}`);
  }

  // 如果 data.doctors 为空但 staff.json 有医师数据，则使用 staff.json 的数据
  if ((!data.doctors || data.doctors.length === 0) && staffDoctors.length > 0) {
    data = Object.assign({}, data, { doctors: staffDoctors });
  }
  ...
  if (!data.contacts || Object.keys(data.contacts).length === 0) {
    try {
      ...
      if (director) contacts.director = { name: director.name, phone: director.phone };
      const advisor = allStaff.find(s => s.role === 'memberAdvisor' || s.role === 'advisor');
      if (advisor) contacts.memberAdvisor = { name: advisor.name, phone: advisor.phone };
      const receptionist = allStaff.find(s => s.role === 'receptionist' || s.role === 'booking');
      if (receptionist) contacts.booking = { phone: receptionist.phone };
      if (Object.keys(contacts).length > 0) {
        data = Object.assign({}, data, { contacts });
      }
    } catch (err) {
      console.error(`⚠️ 读取 staff.json 联系信息失败: ${err.message}`);
    }
  }

  const sections = [];
  const basicInfo = renderTcmBasicInfo(tcmInfo);
...
if (require.main === module) {
  // 读取 setup-state.json 获取当前 propertyType
  const statePath = path.join(__dirname, 'setup-state.json');
  let propertyType = null;

  try {
    const state = loadJSONSafe(statePath);
    propertyType = state.propertyType;
  } catch (_) { /* 忽略 */ }

  // 若 setup-state 中无类型，尝试从 config.json 获取
  if (!propertyType) {
    const config = loadJSONSafe(CONFIG_PATH);
  ...
  if (!propertyType) {
    console.error('❌ 未设置商户类型，请先运行配置向导（setup wizard）');
    process.exit(1);
  }

  // 从 config.json 加载对应业态的数据
  const config = loadJSONSafe(CONFIG_PATH);
  let data = {};
  ...
  switch (propertyType) {
    case 'homestay':
      ...
      if (data.name || config.basic) data.basic = data.basic || config.basic || { name: data.name };
      break;
    case 'apartment':
      data = config.apartment || {};
      if (config.basic) data.basic = data.basic || config.basic;
      break;
    case 'hotel':
      data = config.hotel || {};
      if (config.basic) data.basic = data.basic || config.basic;
    ...
    case 'tcm-clinic':
      data = config.tcm || {};
      data.basic = data.basic || { name: data.clinicName || '中医馆' };
      break;
    default:
      console.error(`❌ 不支持的商户类型：${propertyType}`);
      process.exit(1);
  }

  // 合并通用字段
  if (config.contacts) data.contacts = data.contacts || config.contacts;

  generateKnowledgeBase(data, propertyType);
  console.log(`✅ [${propertyType}] 知识库生成完成`);
}
