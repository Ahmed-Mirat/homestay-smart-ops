...
const TYPE_DATA_FILES = {
  ...
  'tcm-clinic': ['members.json', 'transactions.json', 'inventory.json', 'service-rules.json'],
};
...
function check() {
  ...
  // ▸ 配置状态
  // 4. 基础配置
  const merchantBlock = config[type] || config.homestay || {};
  const merchantName = merchantBlock.name;
  ...
  const dataOk = badFiles === 0 && missingRequired.length === 0;
  let dataDetail;
  if (dataOk) {
...
function render(results) {
  ...
  const merchantBlock = config[type] || config.homestay || {};
  const merchantName = (merchantBlock.name && String(merchantBlock.name).trim()) || typeLabel;
